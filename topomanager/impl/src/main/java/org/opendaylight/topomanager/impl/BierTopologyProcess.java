...
public class BierTopologyProcess<T> implements Callable<ListenableFuture<T>>,  TransactionChainListener {
	...
	protected final DataBroker m_cDataBroker;
	    
    protected static final int MAX_TRANSACTION_OPERATIONS = 100;
    protected static final int OPERATION_QUEUE_DEPTH = 500;
    public final static int  FLAG_READ             =  0;           // 数据操作读标志
    public final static int  FLAG_WRITE            =  1;           // 数据操作写标志

    protected final BlockingQueue<BierTopologyOperation> m_queTopoOperation = new LinkedBlockingQueue<>(OPERATION_QUEUE_DEPTH);
    //protected BindingTransactionChain m_cTxChain = null;
    protected final int        m_iProcessFlag;
    public final T             m_tT;

   
    public BierTopologyProcess(final DataBroker dataBroker, final int iProcessFlag, final T tT) {
        this.m_cDataBroker = Preconditions.checkNotNull(dataBroker);                  // 检查数据是否为空
        //m_cTxChain = this.m_cDataBroker.createTransactionChain(this);           // 创建事务
        m_iProcessFlag = iProcessFlag;
        m_tT = tT;
    }
    ...
    public ListenableFuture<T> call() throws Exception {
        T tObject = m_tT;
        try {
        	BierTopologyOperation op = m_queTopoOperation.take();                     // 获取操作队列中的对象，进行操作
            final ReadWriteTransaction tx = this.m_cDataBroker.newReadWriteTransaction(); // 获取事务
            int ops = 0;                                                          // 记录操作执行条数
            ...
            do {
                ...
                if ( 0 == m_iProcessFlag ) {
                    ...
                    try {
                        tObject = readResult.get().get();          // 获取信息
                    } catch (Exception e) {
                ...
                if (ops < MAX_TRANSACTION_OPERATIONS) {
                    op = m_queTopoOperation.poll();                                  // 取出队列首元素
                } else {
                    op = null;                                          // 操作次数过多，不执行
                }
        ...
        // 返回执行结果
        return Futures.immediateFuture(tObject);
    }

    
    public void enqueueOperation(final BierTopologyOperation task) {
        try {
            m_queTopoOperation.put(task);                                             // 存入队列
    ...
    protected void cleanDataStoreOperQueue() {
        ...
        // 清除事务表
        while (!m_queTopoOperation.isEmpty()) {
            m_queTopoOperation.poll();
        }
    }
    ...
    public void onTransactionChainSuccessful(TransactionChain<?, ?> chain) {
        //NOOP
        System.out.println("Transaction Chain success");
    }
