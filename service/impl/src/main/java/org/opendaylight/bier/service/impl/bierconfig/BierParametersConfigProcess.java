...
public class BierParametersConfigProcess {
    ...
    private boolean checkIpv6BfrPrefixChange(Domain before, Domain after) {
        if (null != after.getBierGlobal().getIpv6BfrPrefix() && null != before.getBierGlobal().getIpv6BfrPrefix()
                && after.getBierGlobal().getIpv6BfrPrefix().equals(before.getBierGlobal().getIpv6BfrPrefix())) {
            return false;
        } else {
            return true;
        }
